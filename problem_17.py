...
longest_abs_path = ""
for root, dirs, files in os.walk("/home/mediaworker/os_walk"):
    if len(root) > len(longest_abs_path):
        longest_abs_path = root
    for each_file in files:
        abs_file_path = os.path.join(root, each_file)
        if len(abs_file_path) > len(longest_abs_path):
            longest_abs_path = abs_file_path

print "longest_abs_path",longest_abs_path
